...
export function Caixa() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6 w-full max-w-full overflow-x-hidden">
      <ModalAlert {...alertState} onClose={closeAlert} />

      {/* Header Banner */}
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-2xl bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-bold shrink-0">
      ...
      {/* Main Banner Card */}
      <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white/80 p-4 sm:p-6 shadow-xl backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/60 space-y-3 sm:space-y-4 w-full max-w-full overflow-hidden">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">
          GESTÃO FINANCEIRA COMPLETA
        </span>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="min-w-0">
            <h2 className="text-lg sm:text-2xl font-black text-slate-900 dark:text-white">Entradas, saídas &amp; saldo</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 hidden sm:block">Acompanhe recebimentos, compras de materiais e acertos entre profissionais.</p>
          </div>
          <button
            onClick={() => setShowModal(true)}
            className="btn-animated inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-2.5 sm:px-5 sm:py-3 text-xs font-black text-white shadow-lg shadow-emerald-500/25 transition hover:opacity-90 w-full sm:w-auto justify-center shrink-0"
          >
            <Plus className="h-4 w-4" /> Nova Movimentação
          </button>
        </div>

        <div className="flex items-center gap-2 pt-2 overflow-x-auto no-scrollbar w-full max-w-full">
          {[
            ...
            <button
              ...
              className={`rounded-2xl px-4 py-2 text-xs font-extrabold transition-all whitespace-nowrap shrink-0 ${
                activeFilter === key
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 w-full max-w-full overflow-hidden">
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          {viewMode === 'grafico' && (
            <div className="flex items-center gap-1 p-1 rounded-2xl bg-slate-200/60 dark:bg-slate-800/80">
              <button
      ...
      {/* KPIs / Gráfico + Lançamentos */}
      <div className="grid gap-4 sm:gap-6 xl:grid-cols-[1fr_1.05fr] w-full max-w-full">
        <div className="rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/60 p-3 sm:p-6 shadow-xl backdrop-blur-md w-full max-w-full overflow-hidden">
          {viewMode === 'valores' ? (
            <div className="grid grid-cols-3 sm:grid-cols-3 gap-1.5 sm:gap-4 w-full">
              {/* KPI: Total Recebido */}
              <div className="rounded-2xl sm:rounded-3xl border border-emerald-500/20 bg-white/80 dark:bg-slate-900/60 p-2.5 sm:p-6 shadow-inner shadow-emerald-500/10 min-w-0 overflow-hidden">
                <div className="flex items-center justify-between mb-1 sm:mb-0">
                  <div className="h-6 w-6 sm:h-8 sm:w-8 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0">
                    <CheckCircle2 className="h-3 w-3 sm:h-4 sm:w-4" />
                  </div>
                </div>
                <div className="text-[11px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 leading-tight mb-1 hidden sm:block">TOTAL RECEBIDO</div>
                <div className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 leading-tight mb-1 sm:hidden truncate">Recebido</div>
                <div className="text-xs sm:text-3xl font-black text-emerald-600 dark:text-emerald-400 leading-tight truncate">
                  R$&nbsp;{totalRecebido.toFixed(2).replace('.', ',')}
                </div>
                <div className="mt-0.5 text-[10px] font-semibold text-slate-400 hidden sm:block">
                  {qtdRecebidos} recebimento(s)
                </div>
              </div>

              {/* KPI: À Receber */}
              <div className="rounded-2xl sm:rounded-3xl border border-amber-500/20 bg-white/80 dark:bg-slate-900/60 p-2.5 sm:p-6 shadow-inner shadow-amber-500/10 min-w-0 overflow-hidden">
                <div className="flex items-center justify-between mb-1 sm:mb-0">
                  <div className="h-6 w-6 sm:h-8 sm:w-8 rounded-full bg-amber-500/20 text-amber-600 dark:text-amber-400 flex items-center justify-center shrink-0">
                    <Clock className="h-3 w-3 sm:h-4 sm:w-4" />
                  </div>
                </div>
                <div className="text-[11px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 leading-tight mb-1 hidden sm:block">À RECEBER</div>
                <div className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 leading-tight mb-1 sm:hidden truncate">A Receber</div>
                <div className="text-xs sm:text-3xl font-black text-amber-600 dark:text-amber-400 leading-tight truncate">
              ...
              {/* KPI: Saídas */}
              <div className="rounded-2xl sm:rounded-3xl border border-purple-500/20 bg-white/80 dark:bg-slate-900/60 p-2.5 sm:p-6 shadow-inner shadow-purple-500/10 min-w-0 overflow-hidden">
                <div className="flex items-center justify-between mb-1 sm:mb-0">
                  <div className="h-6 w-6 sm:h-8 sm:w-8 rounded-full bg-purple-500/20 text-purple-600 dark:text-purple-400 flex items-center justify-center shrink-0">
                    <ArrowDownRight className="h-3 w-3 sm:h-4 sm:w-4" />
                  </div>
                </div>
                <div className="text-[11px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 leading-tight mb-1 hidden sm:block">SAÍDAS</div>
                <div className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 leading-tight mb-1 sm:hidden truncate">Saídas</div>
                <div className="text-xs sm:text-3xl font-black text-purple-600 dark:text-purple-400 leading-tight truncate">
                  ...
                  <div className="space-y-2 sm:space-y-3 w-full max-w-xs">
                    <div className="flex items-center justify-between text-xs font-bold">
                      <span className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                        <span className="h-3 w-3 rounded-full bg-emerald-500 inline-block" /> Total Recebido
                      </span>
        ...
        <div className="rounded-2xl sm:rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-3 sm:p-6 shadow-xl backdrop-blur-xl space-y-3 sm:space-y-4 w-full max-w-full overflow-hidden">
          ...
          <div className="flex flex-col gap-3 border-b border-slate-100 dark:border-slate-800 pb-3 sm:pb-4 w-full">
            <div className="flex items-center justify-between gap-2 min-w-0">
              ...
              <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 hidden sm:block shrink-0">
                Agendamentos confirmados incluídos
              </p>
            </div>
            ...
            <div className="flex items-center gap-1 p-1 rounded-2xl bg-slate-100 dark:bg-slate-800/70 border border-slate-200/50 dark:border-slate-700/50 w-full overflow-x-auto no-scrollbar">
              <button
                ...
                className={`flex-1 min-w-[65px] flex items-center justify-center gap-1 px-2 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-xs font-extrabold transition-all whitespace-nowrap ${
                  statusTab === 'tudo'
                    ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm'
              ...
              <button
                ...
                className={`flex-1 min-w-[80px] flex items-center justify-center gap-1 px-2 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-xs font-extrabold transition-all whitespace-nowrap ${
                  statusTab === 'recebido'
                    ? 'bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 shadow-sm'
                    : 'text-slate-500 dark:text-slate-400'
                }`}
          ...
          {loading ? (
            ...
            <div className="space-y-2 sm:space-y-2.5 max-h-[55vh] sm:max-h-[520px] overflow-y-auto pr-0.5 w-full">
              {movimentacoesFiltradas.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className={`group flex flex-col gap-2 p-2.5 sm:p-3.5 rounded-2xl border transition-all shadow-sm w-full max-w-full overflow-hidden ${
                      isAgendamentoVirtual && !isPago
                        ? 'bg-amber-50/60 dark:bg-amber-900/10 border-amber-300/40 dark:border-amber-700/30'
                        : 'bg-slate-50/80 dark:bg-slate-800/40 hover:bg-white dark:hover:bg-slate-800/90 border-slate-200/60 dark:border-slate-800/80'
                    }`}
                  >
                    {/* Primeira Linha: Ícone + Descrição/Data + Valor/Ações */}
                    <div className="flex items-start justify-between gap-2 w-full min-w-0">
                      <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
                        {/* Ícone de Entrada/Saída */}
                        <div className={`h-7 w-7 sm:h-9 sm:w-9 rounded-xl flex items-center justify-center shrink-0 font-bold ${
                          isEntrada 
                            ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' 
                            : 'bg-rose-500/10 text-rose-600 dark:text-rose-400'
                        }`}>
                          {isEntrada ? <ArrowUpRight className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> : <ArrowDownRight className="h-3.5 w-3.5 sm:h-4 sm:w-4" />}
                        </div>

                        {/* Descrição e Data */}
                        <div className="min-w-0 flex-1 overflow-hidden">
                          <h4 className="font-extrabold text-slate-900 dark:text-white text-xs sm:text-sm truncate leading-tight">
                            {m.descricao}
                          </h4>
                      ...
                      <div className="flex flex-col items-end gap-1 shrink-0 min-w-0">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {/* Botão Dar Baixa — só aparece se pendente */}
                          {!isPago && isEntrada && (
                            <button
                              onClick={() => handleOpenBaixa(m)}
                              title="Dar Baixa"
                              className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-500 hover:text-white text-[10px] font-black uppercase tracking-wide transition-all border border-emerald-500/20"
                            >
                          ...
                          {/* Botão Excluir — oculto para virtuais pendentes */}
                          {!(isAgendamentoVirtual && !isPago) && (
                            <button 
                              onClick={() => handleDeleteEntry(m.id)} 
                              className="p-1 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition"
                              title="Excluir"
                            >
                    ...
                    {/* Segunda Linha: Tags e Status */}
                    <div className="flex items-center justify-between pt-1.5 border-t border-slate-200/40 dark:border-slate-800/40 text-[10px] gap-1 w-full min-w-0">
                      <div className="flex items-center gap-1 flex-wrap min-w-0">
                        {/* Forma de Pagamento */}
                        {renderPaymentBadge(m.forma_pagamento)}

                        {/* Tag de Origem do Agendamento */}
                        {m.categoria === 'agendamento' && (
                          <span className="px-1.5 py-0.5 rounded-lg font-black uppercase tracking-wider bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20 text-[9px] sm:text-[10px]">
                            Agendamento
                          </span>
                        )}

                        {/* Tag de Origem de Material/Insumo */}
                        {m.categoria === 'material' && (
                          <span className="px-1.5 py-0.5 rounded-lg font-black uppercase tracking-wider bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20 text-[9px] sm:text-[10px]">
                            Material
                          </span>
                        )}

                        {/* Tag de Tipo */}
                        <span className={`px-1.5 py-0.5 rounded-lg font-black uppercase tracking-wider text-[9px] sm:text-[10px] ${
                          isEntrada 
                            ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' 
                            : 'bg-rose-500/10 text-rose-600 dark:text-rose-400'
                        }`}>
                      ...
                      {/* Status de Pagamento */}
                      <span className={`inline-flex items-center gap-0.5 px-2 py-0.5 rounded-lg font-black uppercase tracking-wider border shrink-0 text-[9px] sm:text-[10px] ${
                        isPago 
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-2xl border text-xs font-extrabold transition-all ${
                      baixaFormaPagamento === value
