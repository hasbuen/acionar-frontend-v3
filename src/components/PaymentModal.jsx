      ...
      <div className="my-0 w-full max-w-lg max-h-[95vh] flex flex-col overflow-hidden rounded-t-[2rem] border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 text-slate-900 dark:text-slate-100 sm:my-auto sm:rounded-[2.2rem]">
        ...
        <div className="overflow-y-auto px-5 sm:px-7 py-4 sm:py-5 space-y-5">
          <div className="flex items-center justify-between rounded-2xl border border-slate-200/60 bg-slate-50 dark:border-slate-800/60 dark:bg-slate-950/50 p-3.5">
            ...
            <span className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[10px] font-black uppercase text-emerald-600 dark:text-emerald-400 shrink-0">
              Fluxo de Caixa
            </span>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            {/* VALORES */}
            <div className="flex flex-col sm:grid sm:grid-cols-3 gap-3 sm:gap-2.5">
            ...
            {/* OBSERVAÇÕES */}
            <input value={draft.notes} onChange={e => setDraft({ ...draft, notes: e.target.value })} placeholder="Observações do caixa (opcional)" className={`${inputClass} px-4 py-3 sm:py-2.5`} />

            {/* COBRANÇA ONLINE */}
            <div className="space-y-3 rounded-2xl sm:rounded-3xl border border-blue-500/25 bg-blue-500/5 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <span className="block text-[11px] font-semibold leading-relaxed text-slate-500 dark:text-slate-400 mt-0.5">Gera um link para o cliente escolher Pix ou cartão. O Pix exibe o QR Code no checkout.</span>
                </div>
              </div>
            ...
            <button type="submit" className="btn-animated w-full rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 py-4 sm:py-3.5 text-xs font-black text-white shadow-lg shadow-emerald-500/20">Registrar pagamento</button>
          </form>

          {payments.length > 0 && <div className="border-t border-slate-100 dark:border-slate-800 pt-3 pb-2 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center">{payments.length} lançamento(s) já registrado(s) neste atendimento.</div>}
        </div>
      </div>
    </div>
  );
