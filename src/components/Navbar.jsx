...
export function Navbar({ activeTab, setActiveTab }) {
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Bar - Design Unificado Limpo & Sofisticado */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 border-t border-slate-200/80 dark:border-slate-800/80 backdrop-blur-2xl pb-safe-bottom dark:bg-slate-950/95 shadow-[0_-2px_10px_rgba(0,0,0,0.03)] dark:shadow-[0_-2px_10px_rgba(0,0,0,0.3)] transition-colors duration-200">
        <div className="flex h-14 items-center gap-1 px-1.5 w-full overflow-x-auto snap-x snap-mandatory no-scrollbar scroll-x-touch">
          {NAV_ITEMS.map(({ id, label, icon: Icon }) => {
            const isActive = activeTab === id;
            return (
              <button
                key={id}
                onClick={() => handleTabChange(id)}
                className={`flex snap-center shrink-0 min-w-[4.3rem] flex-1 flex-col items-center justify-center py-1 px-1 transition-all duration-200 ${
                  isActive
                    ? 'text-blue-600 dark:text-blue-400 font-extrabold'
                    : 'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200 font-medium'
                }`}
              >
                <Icon id={`nav-icon-mobile-${id}`} className={`h-5 w-5 transition-transform ${isActive ? 'stroke-[2.2px] scale-110 text-blue-600 dark:text-blue-400' : 'stroke-[1.75px]'}`} />
